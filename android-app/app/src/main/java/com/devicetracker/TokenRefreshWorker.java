 ...
 * - Worker runs every 6 hours to ensure timely refresh even with battery
 * optimization
...
public class TokenRefreshWorker extends Worker {
    ...
    // Run every 4 hours for more aggressive refresh attempts
    private static final long WORK_INTERVAL_HOURS = 4;

    // Refresh when less than 3 days (72 hours) remains - gives bigger window
    private static final long REFRESH_THRESHOLD_MS = 3L * 24 * 60 * 60 * 1000;
    ...
    public Result doWork() {
        ...
        // If still no valid expiry, assume token is valid and set expiry for 7 days
        // from now
        if (tokenExpiry == 0) {
        ...
        // Also check if we haven't refreshed in more than 3 days (proactive refresh)
        long timeSinceLastRefresh = now - lastRefreshTime;
        long daysSinceRefresh = timeSinceLastRefresh / (24 * 60 * 60 * 1000);

        if (lastRefreshTime > 0 && daysSinceRefresh >= 3) {
