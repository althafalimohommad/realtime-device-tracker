...
public class TokenExpiryNotification {
    ...
    // Show notification when less than 48 hours remain (day 6 of 7)
    private static final long NOTIFICATION_THRESHOLD_MS = 48 * 60 * 60 * 1000L;

    // Don't show notification more than once per 8 hours
    private static final long NOTIFICATION_COOLDOWN_MS = 8 * 60 * 60 * 1000L;
    ...
    private static void showExpiryWarningNotification(Context context, long hoursRemaining) {
        ...
        if (hoursRemaining <= 1) {
            ...
            timeText = daysRemaining + (daysRemaining == 1 ? " day" : " days");
        }

        String urgency = hoursRemaining <= 12 ? "Soon!" : hoursRemaining <= 24 ? "Soon" : "Tomorrow";

        NotificationCompat.Builder builder = new NotificationCompat.Builder(context, CHANNEL_ID)
                .setSmallIcon(android.R.drawable.ic_dialog_alert)
                .setContentTitle("🔐 Session Expires " + urgency)
                .setContentText("Your session expires in " + timeText + ". Tap to refresh.")
                .setStyle(new NotificationCompat.BigTextStyle()
                        .bigText("Your device tracking session expires in " + timeText + ".\n\n" +
                                "Auto-refresh is being attempted in the background. " +
                                "If it fails, please open the app and tap your profile icon → 'Refresh Token'."))
